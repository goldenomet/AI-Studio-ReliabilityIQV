...
export const Preloader: React.FC<PreloaderProps> = ({ onComplete }) => {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-brand-bg dark:bg-brand-dark"
      initial={{ opacity: 1 }}
      ...
      >
        {letters.map((letter, index) => (
          <motion.span
            key={index}
            variants={child}
            className="text-4xl md:text-6xl lg:text-7xl font-bold font-mono tracking-tight text-brand-dark dark:text-brand-bg"
          >
            {letter === " " ? "\u00A0" : letter}
          </motion.span>
        ))}
