...
export const ContactSection = () => {
  ...
  const [successMessage, setSuccessMessage] = useState('Data Transmitted');
  ...
  const handleConnectGmail = async () => {
    ...
    try {
      ...
      setGmailTokenState(res.accessToken);
      if (res.user.email) {
        setFormData(prev => ({ 
          ...prev, 
          email: prev.email || res.user.email || '',
          firstName: prev.firstName || res.user.displayName?.split(' ')[0] || '',
          lastName: prev.lastName || res.user.displayName?.split(' ').slice(1).join(' ') || ''
        }));
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let sentToGmailDirectly = false;
    const fullName = `${formData.firstName} ${formData.lastName}`.trim();

    try {
      const token = gmailToken || getGmailAccessToken();
      
      // 1. Send via Client-side Gmail REST API if authenticated
      if (token) {
        try {
          await sendFormSubmissionViaGmail(
            token,
            {
              ...
              subject: `[ReliabilityIQ] Inquiry from ${fullName}`
            },
            'reliabilityiqventures@gmail.com'
          );
          sentToGmailDirectly = true;
        } catch (gErr) {
          console.warn("Direct Gmail API dispatch failed, trying backend proxy:", gErr);
        }
      }

      // 2. Post to backend `/api/lead`
      ...
      const resData = await leadResponse.json().catch(() => ({}));
      if (sentToGmailDirectly || resData.deliveredViaGmail) {
        setSuccessMessage("Delivered Directly to Gmail Inbox");
      } else {
        setSuccessMessage("Data Transmitted & Logged");
      }

      setIsSuccess(true);
      setFormData({ firstName: '', lastName: '', email: gmailUser?.email || '', service: 'Web Operations', details: '' });
      ...
      setSuccessMessage("Opening Mail Client");
      setIsSuccess(true);
    } finally {
          ...
          <div className="lg:w-[55%] p-8 md:p-12 lg:p-16 border-b lg:border-b-0 lg:border-r border-border-primary relative transition-colors duration-500">
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center justify-between gap-2 mb-6 flex-wrap">
                ...
                {/* Gmail Integration Status Chip */}
                {gmailUser ? (
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 font-mono text-xs">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                    <Mail size={13} />
                    <span className="truncate max-w-[140px] font-medium">{gmailUser.email}</span>
                    <button 
                      type="button" 
                      onClick={handleDisconnectGmail}
                      className="ml-1 text-[10px] text-text-secondary hover:text-red-400 underline"
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-bg-primary border border-border-primary hover:border-accent text-text-primary text-xs font-mono font-bold transition-all hover:scale-105 active:scale-95 shadow-xs"
                  >
                  ...
                  <AnimatePresence mode="wait">
                    {isSuccess ? (
                      ...
                      >
                        ...
                        <div>
                          <p className="font-black text-xl uppercase tracking-tighter italic">{successMessage}</p>
                          <p className="text-[10px] opacity-70 tracking-widest">GMAIL OAUTH ACTIVE • SUBMISSION RECORDED</p>
                        </div>
                      </motion.div>
                    ) : (
              ...
              <div className="space-y-10 group/terminal">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-2 font-mono text-base font-bold tracking-tighter text-text-primary">
                    ...
                    <a href="tel:+2349065394605" className="hover:text-accent transition-all hover:translate-x-1 inline-block w-fit">+234 906 539 4605</a>
                  </div>
                </div>
