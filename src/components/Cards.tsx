  ...
  return (
    <div className="h-screen sticky top-0 flex items-center justify-center py-16">
       <motion.div 
         whileHover={{ 
           scale: 1.025,
           boxShadow: "0 60px 120px -30px var(--accent-glow), 0 0 50px var(--accent-glow)",
         ...
         className="bg-bg-card p-6 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] dark:shadow-none border border-border-primary flex flex-col md:flex-row gap-6 w-full max-w-4xl h-auto min-h-[280px] md:h-[350px] relative overflow-hidden group transition-colors duration-500 rounded-[30px] md:rounded-[40px] isolate"
       >
         <div className="md:w-1/2 flex flex-col justify-center h-full z-10 w-full">
            <div className="p-3 bg-accent/10 rounded-2xl text-accent w-fit mb-4 md:mb-6 group-hover:bg-accent group-hover:text-white transition-all duration-500">
              <Icon size={24} className="md:w-6 md:h-6" />
            </div>
            <TextEngine text={title} className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3 font-mono tracking-tighter text-text-primary transition-colors duration-500 leading-none" />
            <p className="text-text-secondary transition-colors duration-500 font-mono text-xs leading-relaxed mb-6 md:mb-8 max-w-sm">
              {description}
            </p>
            <button 
              onClick={onAction}
              className="group/btn flex items-center gap-2 text-accent font-mono text-xs md:text-sm font-bold w-fit bg-bg-secondary px-4 py-2 md:px-6 md:py-3 rounded-full hover:bg-accent hover:text-white transition-all duration-300"
            >
         ...
         <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4 w-96 h-96 bg-accent/5 rounded-full blur-[100px] pointer-events-none group-hover:bg-accent/10 transition-colors"></div>
       </motion.div>
    </div>
  )
}
